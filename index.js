...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'END:VCARD'
prefix = '#'//PREFIX BOT
blocked = ['558585245713']//NOMER BLOCK
limitawal = '50'
cr = '*Bot Verificado*'
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Thauzinhoo otário @${num.split('@')[0]}, Vai pela sombra pq bosta no sol seca 😂😂👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["11934660977@s.whatsapp.net"] // Nomer Owner
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        // Options
                        const botName = 'Patinhas⚡'//Nama Bot
	...
	case 'kbbi':
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'audio':
				   ...
				   if (args.length < 1) return client.sendMessage(from, '*❗ Inválido, use #audio pt*', text, {quoted: mek})
                                   if (!isRegister) return reply(mess.only.daftarB)
                                   if (isLimit(sender)) return reply(ind.limitend(pusname))
	    ...
	    case 'kapankah':
               ...
               if (mus1.error) return reply(mus1.error)
                ...
                break
                   ...
                   break
					...
					if (args.length < 1) return reply('❗Inválido, digite #bv 1 para ativar')
					if (Number(args[0]) === 1) {
            ...
            case 'leveling':
                if (!isGroup) return reply(mess.only.group)
                if (!isGroupAdmins) return reply(mess.only.admin)
                if (args.length < 1) return reply('Digite #leveling 1 para ativar o recurso')
                if (args[0] === '1') {
            ...
            break
				        ...
				        uptime = process.uptime()
                                        client.sendMessage(from, `Velocidade: *${latensi.toFixed(4)} _Segundos_*\nDispositivo: *S10 Lite*\nRAM: *4/6GB*\Data: *Telakomsel*\nRede: *WIFI*\nStatus: *Descarregando...*`, text, { quoted: mek})
                                        break
                                /*case 'jadwaltvnow':  
				if (!isRegister) return reply(mess.only.daftarB)
                               reply(mess.wait)
